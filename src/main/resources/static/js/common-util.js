...
CommonJS.Byte = {
    ...
    checkByte: function(obj, nowByteEle, isEucKr) {
        ...
        var _textLen = _textVal.length;

        var _totalByte = 0;
        for (var i=0; i < _textLen; i++) {
            var eachChar = _textVal.charAt(i);
...
CommonJS.Input = {
    ...
    formatHypenPhone: function (inputElement, inputElementStr) {
        if (inputElement !== null) {
            ...
            if (document.querySelector(inputElementStr).length === undefined) {
                var inputElement = document.querySelector(inputElementStr);
                ...
                var inputElement = $(inputElementStr);
...
CommonJS.Masking = {
    ...
    email3: function(str) {
        ...
        if ( CommonJS.Valid.isBlank(originStr) || CommonJS.Valid.isBlank(emailStr[0]) ) {
            ...
            var emailPrefix = originStr.toString().replace(new RegExp('.(?=.{0,' + strLength + '}@)', 'g'), '*').split('@')[0];
            return emailPrefix + '@' + '******';
        }
    },
    /**
